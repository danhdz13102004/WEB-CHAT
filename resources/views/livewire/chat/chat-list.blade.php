<div class="flex flex-col transition-all h-full overflow-hidden" x-data="{ type: 'all', index: @entangle('index') }" x-init="setTimeout(() => {
    {{-- console.log(index); --}}
    conversationElement = document.getElementById('conversation-' + index);
    ...
    <header class="px-3 z-10 bg-white sticky top-0 w-full py-2 ">
        ...
        <div class="flex gap-3 items-center  p-2 bg-white">
            <button @click="type='all'" :class="{ 'bg-blue-100': type == 'all' }"
            ...
            <button @click="type='deleted'" :class="{ 'bg-blue-100': type == 'deleted' }"
                class="inline-flex justify-center items-center rounded-full gap-x-1 text-xs font-medium px-3 lg:px-5 py-1 lg:py-2.5 border">
                Deleted
            </button>
        </div>
    ...
    <main class="overflow-y-scroll overflow-hidden grow h-full relative scroll-ms-8" style="contain: content">
        <ul class="p-2 grid w-full spacey-y-2">
            @if ($conversations)
                @foreach ($conversations as $conversation)
                    <li id="conversation-{{ $conversation->id }}"
                        class="{{ $conversation->id == $selectedConversation?->id ? 'bg-gray-300' : '' }} py-3 hover:bg-gray-50 rounded-2xl dark:hover:bg-gray-700/70 transition-colors duration-150 flex gap-4 relative w-full cursor-pointer px-2 ">
                        <a href="#" class="shrink-0">
                            <x-avatar
                                src="https://api.dicebear.com/6.x/fun-emoji/svg?seed={{ $conversation->getReceiver()->name }}" />
                        </a>
                        ...
                        <aside class="grid grid-cols-12 w-full">
                            ...
                            <a href="{{ route('chat', $conversation->id) }}"
                                ...
                                <div class="flex gap-x-2 items-center">
                                    ...
                                    {{-- double tick  --}}
                                    @if ($conversation->getNewestMsg()?->read_at)
                                        <span>
                                    ...
                                    @php
                                        $lastmsg = $conversation->getNewestMsg(); 
                                    @endphp
                                    ...
                                    <p class="grow truncate text-sm font-[100]">
                                        ...
                                        @elseif($lastmsg->type == 'image')
                                            Image
                                        @else    
                                            Let's get start a conversation
                                        @endif
                                    </p>
                                    ...
                                    {{-- unread count --}}
                                       ...
                                       @php
                                           $cnt = $conversation->countUnReadMessages();
                                       @endphp
                                    @if ($cnt > 0) 
                                        <span
